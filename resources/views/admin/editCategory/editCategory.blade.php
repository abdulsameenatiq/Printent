<section>
    <div class="row g-0">
        ...
        <div class="col-xl-10 col-md-9 col-sm-8 mt-5 category-form">
            <div class="container">
                <div class="info">
                    <h2 class="pt-3">Edit Category</h2>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const storedCategory = JSON.parse(localStorage.getItem('setcategoryData'));

        console.log("categoryid is", storedCategory.id);

        if (storedCategory) {
            // Prefill the name input with the category name
            document.getElementById('name').value = storedCategory.name;

            console.log("updaetd naem is", storedCategory.name);
            console.log("updaetd image is", storedCategory.image);

            // If the category has an image, show it
            if (storedCategory.image) {
                const uploadedImage = document.getElementById('uploadedImage');

                const imageUrl = `${baseUrl}/${storedCategory.image}`;
                console.log("imageUrl", imageUrl);
                uploadedImage.src = imageUrl;

                console.log("image upload", uploadedImage.src);
    ...
    function submitForm(event) {
        ...
        // https://smartprintsa.com/api/category
        fetch(`https://smartprintsa.com/api/updateCategory/${categoryId}`, {
                // `https://smartprintsa.com/api/deleteCategory/${id}`
                method: 'POST',
                body: formData,
                headers: {
            ...
            .then(data => {
                ...
                alert('Category Updated successfully!'); // Show success alert
